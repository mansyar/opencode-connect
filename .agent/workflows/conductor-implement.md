...
4.  **Select Track:**
    - **If a track name was provided:**
      ...
      2.  If a unique match is found, present the following confirmation to the user in the chat and await their response:
          > "I found track '<track_description>'. Is this correct? (yes/no)"
      3.  If no match is found, or if the match is ambiguous, present the following to the user in the chat and await their response:
          > "I couldn't find a unique track matching the name you provided. Did you mean '<next_available_track>'? Or please type the exact track name."
    - **If no track name was provided (or if the previous step failed):**
      ...
      2.  **If a next track is found:**
          - Present the following confirmation to the user in the chat and await their response:
            > "No track name provided. Would you like to proceed with the next incomplete track: '<track_description>'? (yes/no)"
          - If confirmed, proceed with this track. Otherwise, ask the user in the chat and await their response:
            > "Please type the exact name of the track you would like to implement."
...
5.  **Analyze and Update:**
    ...
    ii. **Propose and Confirm Changes:** If an update is needed: - **Announce:** Briefly state that updates are proposed. - **Ask for Approval:** Present the proposed updates (in a diff format) directly in the chat message so the user can review them in context, and await their response: > **Product Definition Update** > Please review the proposed updates to the Product Definition below. Do you approve? (yes/no) > > --- > > <Insert Proposed product.md Updates/Diff Here>
    ...
    ii. **Propose and Confirm Changes:** If an update is needed: - **Announce:** Briefly state that updates are proposed. - **Ask for Approval:** Present the proposed updates (in a diff format) directly in the chat message so the user can review them in context, and await their response: > **Tech Stack Update** > Please review the proposed updates to the Tech Stack below. Do you approve? (yes/no) > > --- > > <Insert Proposed tech-stack.md Updates/Diff Here>
    ...
    iii. **Propose and Confirm Changes:** If the conditions are met: - **Announce:** Briefly state that updates are proposed. - **Ask for Approval:** Present the proposed changes (in a diff format) directly in the chat message, including a clear warning, and await their response: > **⚠️ Product Guidelines Update (Critical)** > WARNING: This is a sensitive action as it impacts core product guidelines. Please review the proposed changes below. Do you approve these critical changes? (yes/no) > > --- > > <Insert Proposed product-guidelines.md Updates/Diff Here>
...
3.  **Handle User Response:**
    ...
    - **If user chooses "Delete":**
      i. **CRITICAL WARNING:** Before proceeding, present the following warning to the user in the chat and await their confirmation: > ⚠️ WARNING: This will permanently delete the track folder and all its contents. This action cannot be undone. Are you sure? (yes/no)
      ...
      d. **Announce Success:** Announce: "Track '<track_description>' has been permanently deleted." - **If 'no'**:
