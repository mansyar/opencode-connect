...
3.  **Interaction Paths:**
    - **PATH A: Direct Confirmation**
      ...
      2.  Present the following confirmation to the user in the chat and await their response:
          > "You asked to revert the [Track/Phase/Task]: '[Description]'. Is this correct? (yes/no)"
      3.  If "yes", establish this as the `target_intent` and proceed to Phase 2. If "no", ask the user in the chat and await their response:
    ...
    - **PATH B: Guided Selection Menu**
      ...
      2.  **Present a Unified Hierarchical Menu:** Present the identified items as numbered choices in the chat and await the user's response. **CRITICAL:** You MUST limit the list to a maximum of 4 items. Group them by Track in the description if possible.
          > **Select Item**
          > I found multiple in-progress items (or recently completed items). Please choose which one to revert:
          >
          > 1. [Item 1] - [Track description]
          > 2. [Item 2] - [Track description]
          >    ...
          >    Or type "other" to specify a different target.
      3.  **Process User's Choice:**
          ...
          - If the user selects "Other", ask a follow-up question in the chat to find the correct target.
...
2.  **Final Go/No-Go:** Present the following choices to the user in the chat and await their response:

    > **Confirm Plan**
    > Do you want to proceed with the drafted plan?
    >
...
3.  **Process User Choice:**
    ...
    - If "Revise", ask the user in the chat and await their response:
      > "Please describe the changes needed for the revert plan."
